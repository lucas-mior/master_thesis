...
\makenoidxglossaries{}
\UFSCaddSymbol{Emin}{\ensuremath{E_{min}}}{Minimum energy} % chktex 35
...
\UFSCaddSigla{ABNT}{%
  \EngAndPor{Brazilian Association of Technical Standards}%
            {Associação Brasileira de Normas Técnicas}%
}
\UFSCaddSigla{IBGE}{%
  \EngAndPor{Brazilian Institute of Geography and Statistics}%
...
\ABNTEXorientador[\EngAndPor{Supervisor}{Orientador}]{%
  \brazilword{Nome completo do Orientador}, \EngAndPor{Phd.}{Dr.}%
}
...
\UFSCcoordenador[\EngAndPor{Coordinator}{Coordenador}]{%
  \brazilword{Nome do Coordenador}, \EngAndPor{Phd.}{Dr.}%
}
...
\ABNTEXtipotrabalho{%
  \EngAndPor{Bachelor\s Thesis}{Trabalho de Conclusão de Curso}%
}
...
% O preambulo deve conter
% tipo do trabalho, objetivo, nome da instituição e a área de concentração.
\ABNTEXpreambulo{%
  \EngAndPor{%
...
\UFSCaddKeyword{palavrasChaveIngles}   {Keyword 1}
\UFSCaddKeyword{palavrasChavePortugues}{Palavra~=Chave 1}

\UFSCaddKeyword{palavrasChaveIngles}   {Keyword 2}
\UFSCaddKeyword{palavrasChavePortugues}{Palavra~=Chave 2}

\UFSCaddKeyword{palavrasChaveIngles}   {Keyword 3}
\UFSCaddKeyword{palavrasChavePortugues}{Palavra~=Chave 3}
...
% \UFSCaddToIncludeOnly{03_fichacatalografica.tex}
% \UFSCaddToIncludeOnly{04_folhadeaprovacao.tex}
% \UFSCaddToIncludeOnly{06_agradecimentos.tex}
% \UFSCaddToIncludeOnly{07_epigrafe.tex}
% \UFSCaddToIncludeOnly{08_resumos.tex}

% Part 1
% \UFSCaddToIncludeOnly{15a_intro.tex}
% \UFSCaddToIncludeOnly{15b_chapter_1.tex}
% \UFSCaddToIncludeOnly{15b_chapter_2.tex}

% Part 2
% \UFSCaddToIncludeOnly{15z_conclusion.tex}
...
% \ifnum\StrLen{\UFSCincludeOnlyFileList}>0\relax
%   \includeonly{\UFSCincludeOnlyFileList}
% \fi
% \forcedincludetrue
...
\ifforcedinclude\else
  % Fix the \UFSCtextPreliminaryContents not showing up when @twoside is off
  ...
  \ifadvisor\else
    % Manually add the `\UFSCtextPreliminaryContents` to the Table of
    % Contents here to keep the hyper link pointing to the beginning of
    % the page, instead of the beginning of `\UFSCtextPreliminaryContents`
    ...
    \UFSCwhenListIsNotEmpty{\UFSCtextPreliminaryListName}{%
      \begin{KeepFromToc}
        \UFSCtextPreliminaryContents{}
      \end{KeepFromToc}
    }
  ...
  \include{03_fichacatalografica.tex}
...
\ifforcedinclude\else
  \cleardoublepage{}
\fi
\include{04_folhadeaprovacao.tex}
...
\ifforcedinclude\else
{
  ...
  \fi
  % q/list-of-figures-and-tables-when-there-are-no-figures-or-tables
  \UFSCwhenListIsNotEmpty{\listfigurename}{%
  ...
  \fi
  % q/list-of-figures-and-tables-when-there-are-no-figures-or-tables
  \UFSCwhenListIsNotEmpty{\listofquadrosname}{%
  ...
  \fi
  % q/list-of-figures-and-tables-when-there-are-no-figures-or-tables
  \UFSCwhenListIsNotEmpty{\listtablename}{%
  ...
  \fi
  % q/list-of-figures-and-tables-when-there-are-no-figures-or-tables
  \UFSCwhenListIsNotEmpty{\lstlistlistingname}{%
...
\begin{KeepFromToc}
  ...
  % q/adding-color-to-table-of-contents-and-section-headings
  {
    ...
    % q/list-of-figures-and-tables-when-there-are-no-figures-or-tables
    \UFSCwhenListIsNotEmpty{\contentsname}{%
...
\begin{abntexEnvApendicesenv}
  \pdfbookmark[-1]{\ABNTEXapendicesname}{anchor_appendice}
  ...
  \include{18_apendice_a.tex}
  \chapter{appendiceb}
    My appendice B. Add some text here. Textminus: \textminus{} aaa.
...
\begin{abntexEnvAnexos}
  \pdfbookmark[-1]{\ABNTEXanexosname}{anchor_annex}
  ...
  \include{19_anexo_a.tex}
  \include{19_anexo_b.tex}
\end{abntexEnvAnexos}
