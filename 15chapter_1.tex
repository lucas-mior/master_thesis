...
inferências de outras verdades (dependentes de teoria). -- \showfont
...
\begin{teorema}[Teorema de Pitágoras -- \showfont]
    ...
    igual à soma dos quadrados dos comprimentos dos catetos -- \showfont.
\end{teorema}
...
tipos dessas afirmações -- \showfont:
...
\begin{description}
    ...
    \item[Lema:] Um Lema é um ``pré-teorema'', um teorema que serve para ajudar
        ...
        mesmo que não os usem para provar alguma outra coisa. -- \showfont
...
obviamente, num sentido diferente do aqui já descrito). -- \showfont
...
\subsection{Lema}
    ...
    é arbitrária\footnote{Wikipédia -- \showfont} -- \showfont.
...
\begin{lema}
    ...
    a real number $r$ such that $b=ra$. -- \showfont
\end{lema}
...
\begin{definicao}[Fibration -- \showfont]
    ...
    homotopy lifting property for every space $X$. -- \showfont
\end{definicao}
...
\begin{description}
    \item[definition] boldface title, romand body. Commonly used in definitions,
        conditions, problems and examples. -- \showfont
...
das finais: ``Amor é fogo que arde sem se ver''. -- \showfont
...
a destacar o expoente, sem espaço entre elas e com fonte menor -- \showfont
\textcite[5.2.1]{NBR14724:2011}. -- \showfont}\footnote{Caso uma série de notas
...
no corpo do texto. -- \showfont}\footnote{Verifique se os números do expoente
    possuem uma vírgula para dividi-los no corpo do texto. -- \showfont}.
...
\begin{figure}[htb]
    \caption[Gráfico produzido em Excel e salvo como PDF]{Gráfico produzido em
             Excel e salvo como PDF -- \showfont}
    \label{fig_grafico}
    \begin{center}
...
de posicionamento como [htb] -- \showfont:
...
topológicos. -- \showfont}},label={lst:leituradadossim}]
...
\begin{lstlisting}[
language=Python,caption={[Python example]{Python example -- \showfont}}
]
import numpy as np
...
Texmaker\footnote{\url{http://www.xm1math.net/texmaker/} -- \showfont}, entre
...
\begin{lstlisting}
[language=TeX,caption={[TeX example]{TeX example -- \showfont}}]
Ao nomear a \autoref{tab-nivinv} e a \autoref{fig_circulo}, apresentamos um
exemplo de remissão interna, que também pode ser feita quando indicamos o
\autoref{cap_exemplos}, que tem o nome \emph{\nameref{cap_exemplos}}. O número
do capítulo indicado é \ref{cap_exemplos}, que se inicia à
...
porque no Português não temos a palavra ``subsubseção'' -- \showfont.
...
da primeira palavra da primeira linha -- \showfont}
% ---
...
\textit{Text in English language. This environment switches all
    ...
    by using the language name `nohyphenation'. -- \showfont}
\end{otherlanguage*}
...
\url{http://internacional.elpais.com/
     internacional/2013/02/17/actualidad/1361102009_913423.html} -- \showfont}:
...
renuncia. El Papa se centra en la crítica al materialismo}} -- \showfont.
...
de documentos produzidos com \abnTeX{} -- \showfont.
...
{Tabela de conversão de acentuação. -- \showfont}
