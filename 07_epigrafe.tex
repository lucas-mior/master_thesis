\UFSCaddToTextPreliminaryContent{\EngAndPor{Epigraph}{Epigrafe}}

\begin{abntexEnvEpigrafe}
\vspace*{\fill}

\begin{flushright}
  \EngAndPor{
    \textit{``Learn from yesterday, live for today, hope for tomorrow.  
             The important thing is not to stop questioning.''}\\ Albert Einstein
  }{
    \textit{``Aprenda com o ontem, viva para o hoje, tenha esperança para o amanhã.  
             O importante é não parar de questionar.''}\\ Albert Einstein
  }
\end{flushright}

\begin{flushright}
  \EngAndPor{
    \textit{``The true sign of intelligence is not knowledge but imagination.''}\\ Albert Einstein
  }{
    \textit{``O verdadeiro sinal de inteligência não é o conhecimento,  
             mas a imaginação.''}\\ Albert Einstein
  }
\end{flushright}

\begin{flushright}
  \EngAndPor{
    \textit{``Peace cannot be kept by force; it can only be achieved by understanding.''}\\ Albert Einstein
  }{
    \textit{``A paz não pode ser mantida pela força;  
             ela só pode ser alcançada pelo entendimento.''}\\ Albert Einstein
  }
\end{flushright}

\begin{flushright}
  \EngAndPor{
    \textit{``Whoever is careless with the truth in small matters  
             cannot be trusted with important matters.''}\\ Albert Einstein
  }{
    \textit{``Quem é descuidado com a verdade em pequenas coisas  
             não pode ser confiável em assuntos importantes.''}\\ Albert Einstein
  }
\end{flushright}

\begin{flushright}
  \EngAndPor{
    \textit{``Extraordinary claims require extraordinary evidence.''}\\ Carl Sagan
  }{
    \textit{``Afirmações extraordinárias exigem evidências extraordinárias.''}\\ Carl Sagan
  }
\end{flushright}

\begin{flushright}
  \EngAndPor{
    \textit{``Catholic, which I was until I reached the age of reason.''}\\ George Carlin
  }{
    \textit{``Católico, o que eu fui até chegar à idade da razão.''}\\ George Carlin
  }
\end{flushright}

\begin{flushright}
  \EngAndPor{
    \textit{``We made too many wrong mistakes.''}\\ Yogi Berra
  }{
    \textit{``Cometemos muitos erros errados.''}\\ Yogi Berra
  }
\end{flushright}

\end{abntexEnvEpigrafe}
